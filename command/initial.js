...
function copymumuConfigJS(){
  figlet('mumu cli', function(err, data) {
    if(err){
      console.log(chalk.red('Some thing about figlet is wrong!'));
    }
    console.log(chalk.yellow(data));
    let targetFilePath = path.resolve('mumu.config.js');
    let templatePath = path.join(__dirname,'../tpl/mumu.config.js');
    ...
    inquirer.prompt([
      {
        name:'init-confirm',
        type:'confirm',
        message:`mumu.config.js 已经存在，是否覆盖?`,
